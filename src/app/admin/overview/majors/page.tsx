...
export default function Overview() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 2. Fetch Aggregated Statistics (avg_major)
            ...
            // 3. Fetch Faculties & Majors & Teachers
            ...
            const { data: teachersData } = await supabase.from('teachers').select('id, teacher_name');
            ...
            if (majorsData) {
                ...
                majorsData.forEach(m => majorMap.set(m.id, m.major_name));
            }
            ...
            // Fetch Teacher-Major Links via Rooms
            // Major -> Rooms -> TeacherRelationship -> Teachers
            // We need to map Teacher ID -> Major ID
            const { data: roomsData } = await supabase.from('rooms').select('id, major_id');
            const { data: relData } = await supabase.from('teacher_relationship').select('teacher_id, room_id');

            const teacherMajorMap = new Map<number, number>(); // teacher_id -> major_id (one-to-one assumption for simplification, or latest)

            // Build Teacher -> Major Map
            if (roomsData && relData) {
                ...
                relData.forEach(rel => {
                    ...
                    if (mId && rel.teacher_id) {
                        teacherMajorMap.set(rel.teacher_id, mId);
                    }
                });
            }

            // 4. Fetch Participation Counts (Filtered by Faculty & Major)
            // Re-build a robust Teacher -> Major map
            const fullTeacherMajorMap = new Map<number, number>(); // TeacherID -> MajorID

            // 4.1 From Teachers Table (Direct Room)
            teachersData?.forEach(t => {
                // teachersData only has id, name. We need room_id.
                // Re-fetch teachers with room_id to be safe or use what we have if we fetched it earlier?
                // The earlier fetch 'teachersData' (line 139) only selected id, teacher_name.
                // We need to fetch full teacher info or relying on a new query.
            });

            // Let's do a dedicated fetch for counting to be clean and accurate
            const { data: allTeachersData } = await supabase.from('teachers').select('id, room_id');
            const { data: allTeacherRels } = await supabase.from('teacher_relationship').select('teacher_id, room_id');
            const { data: allRooms } = await supabase.from('rooms').select('id, major_id');
            const { data: allMajors } = await supabase.from('majors').select('id, faculty_id');

            const roomToMajor = new Map<number, number>();
            allRooms?.forEach(r => roomToMajor.set(r.id, r.major_id));

            const majorToFaculty = new Map<number, number>();
            allMajors?.forEach(m => majorToFaculty.set(m.id, m.faculty_id));

            // Map Teacher -> Set of Majors they belong to
            const teacherMajorsSet = new Map<number, Set<number>>();

            const addMajorToTeacher = (tId: number, rId: number | null) => {
                if (!rId) return;
                const mId = roomToMajor.get(rId);
                if (mId) {
                    if (!teacherMajorsSet.has(tId)) teacherMajorsSet.set(tId, new Set());
                    teacherMajorsSet.get(tId)?.add(mId);
                }
            };

            allTeachersData?.forEach(t => addMajorToTeacher(t.id, t.room_id));
            allTeacherRels?.forEach(rel => addMajorToTeacher(rel.teacher_id, rel.room_id));

            // Determine Valid Teachers based on Filter
            const validTeacherIds = new Set<number>();

            // If no filter selected, use ALL teachers from the main table
            if (!selectedFaculty && !selectedMajor) {
                allTeachersData?.forEach(t => validTeacherIds.add(t.id));
            } else {
                // If filter active, only check teachers with known majors
                teacherMajorsSet.forEach((majors, tId) => {
                    let isValid = false;
                    for (const mId of Array.from(majors)) {
                        // Check Major Filter
                        if (selectedMajor) {
                            if (mId === selectedMajor) {
                                isValid = true;
                                break;
                            }
                        }
                        // Check Faculty Filter
                            ...
                            const fId = majorToFaculty.get(mId);
            ...
            // Fetch Actual Participation
            const { data: uniqueTeachers } = await supabase
                .from('assessment_answer')
                .select('teacher_id')
                .eq('around_id', selectedRound);

            let filteredParticipatedTeachers = 0;
            if (uniqueTeachers) {
                const uniqueSet = new Set(uniqueTeachers.map(u => u.teacher_id));
                uniqueSet.forEach(tId => {
                    if (validTeacherIds.has(tId)) {
                        filteredParticipatedTeachers++;
                    }
                });
            }

            // Student Counts (simplified for now, strictly speaking should also filter by major)
            ...
            setTotalTeachersInSystem(validTeacherIds.size);
            setParticipatedStudents(new Set(uniqueStudents?.map(x => x.student_id)).size);
            setParticipatedTeachers(filteredParticipatedTeachers);
            ...
            // Filter avgData (if major is selected)
            const filteredAvgData = selectedMajor
                ? avgData?.filter(r => r.major_id === selectedMajor)
                : avgData;
            ...
            filteredAvgData?.forEach(row => {
                ...
                mS.sum += totalScore;
                mS.count += respondents;

                globalSum += totalScore;
                globalCount += respondents;
            });

            // Prepare Bar Chart Data (Majors OR Teachers)
            let computedChartData: EntityScore[] = [];

            if (selectedMajor) {
                // Show TEACHERS in Selected Major
                const { data: avgTeacherData } = await supabase
                    .from('avg_teacher')
                    .select('*')
                    .eq('around_id', selectedRound);
                ...
                avgTeacherData?.forEach(row => {
                    const tId = row.teacher_id;
                    // Check if teacher belongs to selected major
                    if (teacherMajorMap.get(tId) === selectedMajor) {
                        if (!teacherStats.has(tId)) teacherStats.set(tId, { sum: 0, count: 0 });
                        const tS = teacherStats.get(tId)!;
                        tS.sum += Number(row.total_score);
                        tS.count += row.respondent_count;
                    }
                });

                computedChartData = Array.from(teacherStats.entries()).map(([tId, stats]) => {
                ...
                // Show MAJORS (Filtered by Faculty if selected)
                computedChartData = Array.from(majorStats.entries()).map(([mId, stats]) => {
                    // Check Faculty Filter
                    const mOption = majorsData?.find(m => m.id === mId);
                    if (selectedFaculty && mOption?.faculty_id !== selectedFaculty) return null;
                ...
                }).filter(Boolean) as EntityScore[];

                computedChartData.sort((a, b) => b.score - a.score);
            }
            ...
            // E. Prepare Major Data for Bar Chart -> Moved above
    ...
    // Auto-select Major based on Faculty
    useEffect(() => {
        if (selectedFaculty && majors.length > 0) {
            ...
            if (availableMajors.length > 0) {
                // If current selection is invalid or null, select the first one
    ...
    }, [selectedRound, selectedMajor, selectedFaculty]);
                            ...
                            onClick={() => {
                                if (selectedRound) {
                                    const printData = {
                                        ...
                                        domainData,
                                        questions,
                                        chartData, // Already computed (Majors or Teachers)
                                        selectedFaculty,
                                        selectedMajor,
                                        timestamp: Date.now()
                                    };
                                    localStorage.setItem('printData_majors', JSON.stringify(printData));
                                    const url = `/admin/overview/majors/print?round=${selectedRound}${selectedFaculty ? `&faculty=${selectedFaculty}` : ''}${selectedMajor ? `&major=${selectedMajor}` : ''}`;
