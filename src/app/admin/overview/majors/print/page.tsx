...
export default function OverviewPDF() {
    ...
    const majorIdParam = searchParams.get('major');
    ...
    const [selectedMajor, setSelectedMajor] = useState<number | null>(majorIdParam ? Number(majorIdParam) : null);
    ...
    useEffect(() => {
        const localDataString = localStorage.getItem('printData_majors');
        if (localDataString) {
            try {
                ...
                if (Date.now() - localData.timestamp < 30 * 60 * 1000) {
                    ...
                    setChartData(localData.chartData);

                    if (localData.selectedFaculty) setSelectedFaculty(localData.selectedFaculty);
                    if (localData.selectedMajor) setSelectedMajor(localData.selectedMajor);
    ...
    const chartTitle = selectedMajor
        ? "ผลการประเมินรายบุคคล (Ranking)"
        : "ผลการประเมินรายสาขา (Ranking)";
    ...
    return (
        <div className="bg-white min-h-screen p-8 text-slate-900 leading-normal w-[210mm] mx-auto print:p-0 print:w-full">
            ...
            <div className="flex items-center justify-between mb-8 border-b pb-6 border-slate-200">
                <div>
                    <h1 className="text-3xl font-black text-slate-900">รายงานผลการประเมิน (สาขาวิชา)</h1>
