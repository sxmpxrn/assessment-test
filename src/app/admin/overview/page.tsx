    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const { data: uniqueStudents } = await supabase
                ...
                .select('student_id')
                .eq('around_id', selectedRound)
                .neq('score_value', null); // Simplified check
            ...
            // E. Prepare Faculty Data for Bar Chart
            const computedFacultyData: FacultyScore[] = Array.from(facultyStats.entries()).map(([fId, stats]) => {
                const avg = stats.count > 0 ? stats.sum / stats.count : 0;
                return {
            ...
            setFacultyData(computedFacultyData);
                ...
                <div className="max-w-7xl mx-auto flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto overflow-x-auto pb-1 sm:pb-0">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-2xl text-sm font-bold hover:bg-emerald-700 hover:shadow-xl hover:shadow-emerald-600/20 active:scale-95 transition-all disabled:opacity-50 w-full sm:w-auto whitespace-nowrap"
                        >
                            {isCalculating ? <Loader2 className="animate-spin" size={18} /> : <RefreshCw size={18} />}
                        ...
                        <button
                            onClick={() => {
                                if (selectedRound) {
                                    ...
                                    localStorage.setItem('printData_overview', JSON.stringify(printData));
                                    const url = `/admin/overview/print?round=${selectedRound}`;
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                        <div>
                            ...
                            <p className="text-slate-400 text-sm mt-1 font-medium">
                                จัดลำดับผลการประเมินแยกตามหน่วยงาน
                            </p>
                        </div>
