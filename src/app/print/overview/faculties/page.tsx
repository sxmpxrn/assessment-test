        ...
        if (localDataString) {
            try {
                ...
                if (Date.now() - localData.timestamp < 30 * 60 * 1000) {
                    ...
                    setLoading(false);
                    // Clear it so future refreshes might fetch fresh data if needed, or keep it. 
                    // Keeping it allows refresh to work seamlessly.
                    return;
                }
            } catch (e) {
            ...
            <div className="max-w-7xl mx-auto pt-8 space-y-10 relative z-10">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-baseline gap-3 mt-2">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xl text-slate-400 font-bold">/ 5.00</span>
                ...
                <div className="bg-white p-8 rounded-xl border border-slate-300 break-inside-avoid">
                    ...
                    {facultyData.length > 0 ? (
                        <div className="w-full" style={{ height: `${Math.max(facultyData.length * 50, 300)}px` }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                    data={facultyData.map((d, i) => ({ ...d, rank: i + 1 }))}
                                    layout="vertical"
                                    margin={{ top: 0, right: 60, left: 240, bottom: 0 }}
                                    barGap={10}
                                >
                                    ...
                                    <YAxis
                                        ...
                                        tick={({ x, y, payload, index }) => {
                                            ...
                                            // Ensure we don't have too many lines overlapping
                                            const displayLines = lines.slice(0, 2);
                                            if (lines.length > 2) displayLines[1] += "...";

                                            return (
                                                <g transform={`translate(${x - 230},${y})`}>
                                                    <text
                                                        x={0}
                                                        y={-10}
                                                    ...
                                                    {displayLines.map((line, i) => (
                                                        <text
                                                            ...
                                                            x={0}
                                                            y={8 + (i * 16)}
                                                            className="fill-slate-800 text-sm font-bold uppercase tracking-tight"
                                                        >
                                                            {line}
                                                        </text>
                                                    ))}
