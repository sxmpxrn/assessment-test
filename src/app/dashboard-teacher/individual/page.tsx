    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [
                ...
                supabase.from('rooms').select('id, major_id, room_code').order('room_code'),
            ...
            // Identify Current User Role & ID (Secure RPC)
            const { data: roleName } = await supabase.rpc('current_user_role');
            const { data: myDbId } = await supabase.rpc('get_my_db_id');

            // Filter Rooms if Teacher
            let availableRooms = roomsData || [];
            if (roleName === 'teacher' && myDbId && relData) {
                const myRoomIds = new Set<number>();
                const myIdVal = Number(myDbId);
                relData.forEach(r => {
                    if (r.teacher_id === myIdVal && r.room_id) {
                        myRoomIds.add(r.room_id);
                    }
                });
                availableRooms = availableRooms.filter(r => myRoomIds.has(r.id));
            }

            if (availableRooms) {
                setRooms(availableRooms.map(r => ({ id: r.id, code: r.room_code || `Room ${r.id}` })));
            }
            ...
            const teacherRoomMap = new Map<number, number>(); // teacher_id -> room_id
            relData?.forEach(rel => {
            ...
            const teacherMajorMap = new Map<number, number>();
            relData?.forEach(rel => {
                if (rel.room_id && rel.teacher_id) {
            ...
            if (teachersData) {
                ...
                // Auto-select My Room
                if (roleName === 'teacher' && availableRooms.length > 0) {
                    setSelectedRoom((prev) => {
                        // Allow -1 (All Rooms) to persist
                        if (prev === -1) return -1;

                        const isValid = availableRooms.some(r => r.id === prev);
                        // Default to first room if invalid/null, unless we want to default to All? 
                        // Let's default to first specific room for now as per previous behavior, 
                        // but allow -1 if user selected it.
            ...
            rawAnswers?.forEach(row => {
                ...
                if (roleName === 'teacher' && myDbId) {
                    if (tId !== Number(myDbId)) return;
                ...
                const isRelevantForQuestionStats = selectedTeacher ? (tId === selectedTeacher) : true;
            ...
            // E. Teacher Data (Bar Chart) - Showing Top Teachers
            ...
            if (selectedTeacher) {
                ...
                finalPeerData = computedTeacherData
                    ...
                    .map(t => ({ ...t, isCurrent: t.name === teacherMap.get(selectedTeacher) }));
            ...
            // G. Comments
            const targetTeacherId = (roleName === 'teacher' && myDbId) ? Number(myDbId) : selectedTeacher;
            ...
            if (targetTeacherId && selectedRound) {
                ...
                if (commentsData) {
                    ...
                    commentsData.forEach((c: any) => {
                        ...
                        const sectionVal = details?.find(d => d.id === c.question_id)?.section2 || '-';
            ...
            <div className="bg-white/70 backdrop-blur-xl border-b border-white/40 sticky top-0 z-40 px-4 md:px-6 py-4 transition-all shadow-sm">
                <div className="max-w-7xl mx-auto flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto overflow-x-auto pb-1 sm:pb-0">
                        {/* Room Dropdown (Replaces Teacher Dropdown) */}
                        <div className="relative group min-w-[200px]">
                            <select
                                value={selectedRoom ?? -1}
                            ...
                            >
                                <option value="-1">ทั้งหมด (All Rooms)</option>
                        ...
                        <button
                            onClick={() => {
                                if (selectedRound) {
                                    ...
                                    localStorage.setItem('printData_individual_teacher', JSON.stringify(printData));
                                    const url = `/dashboard-teacher/individual/print?round=${selectedRound}${selectedTeacher ? `&teacher=${selectedTeacher}` : ''}`;
